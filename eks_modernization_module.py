    ...
    def step6_review_validate():
        ...
        with col1:
            if st.button("📄 Export to Word", use_container_width=True):
                ...
                doc_content = doc_generator.generate_word_doc(spec)
                
                # Provide download button for the document
                st.download_button(
                    label="⬇️ Download Document",
                    data=doc_content,
                    file_name=f"eks_architecture_{spec.project_name.replace(' ', '_')}.md",
                    mime="text/markdown",
...
"""
        ...
        # Add node group details
        for i, ng in enumerate(spec.node_groups, 1):
            doc_content += f"""
#### Node Group {i}: {ng.get('name', f'ng-{i}')}
- **Instance Type:** {ng.get('instance_type', 't3.medium')}
- **Capacity:** {ng.get('min_size', 1)} - {ng.get('max_size', 5)} nodes
- **Capacity Type:** {ng.get('capacity_type', 'ON_DEMAND')}
- **Disk Size:** {ng.get('disk_size', 100)} GB
"""
...
---
*Generated by EKS Design Hub on {datetime.now().strftime('%Y-%m-%d %H:%M')}*
"""
        
        # Return content directly instead of saving to file
        # Streamlit will handle the download via st.download_button
        return doc_content
    ...
    if 'design_spec' in st.session_state:
        ...
        with col1:
            if st.button("📄 Export to Word", use_container_width=True):
                ...
                doc_content = doc_gen.generate_word_doc(spec)
